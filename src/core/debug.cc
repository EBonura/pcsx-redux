...
bool PCSX::Debug::triggerBP(Breakpoint* bp, uint32_t address, unsigned width, const char* cause) {
    ...
    m_step = STEP_NONE;
    // Suppress per-step chatter (and the editor-focus event) when a
    // harness has asked for quiet mode — otherwise every single-step
    // spams stdout and floods the pipe buffer.
